<div class="card">
    ...
    <form class="form-horizontal" id="formSubmit" method="post"
        action="{{ isset($data['post']) ? route('product.update', ['product' => $data['post']->id]) : route('product.store') }}"
            ...
            <div class="row">
                <div class="col-md-8">
                    @isset($data['post'])
                        <div class="row">
                            <div class="col-md-6">
                                @include('phobrv::input.inputSelect', [
                                    'label' => 'Status',
                                    'key' => 'status',
                                    'array' => ['1' => 'Normal', '2' => 'Hot', '0' => 'Disable'],
                                ])
                            </div>
                            <div class="col-md-6">
                        ...
                        {{ Form::textarea('content', old('content', isset($data['post']) && $data['post'] ? $data['post']->content : ''), ['class' => 'form-control', 'rows' => '20']) }}
                    </div>
                    <div class="col-md-4">
                                    ...
                                    @foreach ($data['group'] as $g)
                                        <li>
                                            <input type="checkbox" name="group[]" value="{{ $g->id }}"
                                                @if (in_array($g->id, $data['arrayGroupID'])) checked @endif> {{ $g->name }}
                                        </li>
                                        @if (isset($g->child))
                                            @foreach ($g->child as $c)
                                                <li style="padding-left: 30px;">
                                                    <input type="checkbox" name="group[]" value="{{ $c->id }}"
                                                        @if (in_array($c->id, $data['arrayGroupID'])) checked @endif> {{ $c->name }}
                                                </li>
                                            @endforeach
                                        @endif
                                    @endforeach
